...
function refresh_group_list() {
    ...
    request.open("GET","/ajax/refresh_group_list.php?useronly=true");
    console.log(request);
...
/* function refresh_sidenav_dashboard(){
    const request = new XMLHttpRequest();

    request.open("GET","../../includes/include-sidenav-dashboard.php");
    console.log(request);
    request.onreadystatechange = function(){
        ...
        if(request.readyState === 4){
            ...
            console.log(res);
            const res = request.responseText;
            const div = document.getElementById("sidenav-reload");
            div.innerHTML = res;
        }
...
function join_group(id_group){
    if(confirm('Êtes vous sur de vouloir rejoindre ce groupe ?')){
        const request = new XMLHttpRequest();

        request.open("GET","/ajax/join_group.php?id_group="+id_group);
...
function decline_group(id_group){
    if(confirm('Êtes vous sur de vouloir supprimer ce groupe ?')){
        const request = new XMLHttpRequest();

        request.open("GET","/ajax/quit_group.php?id_group="+id_group);
